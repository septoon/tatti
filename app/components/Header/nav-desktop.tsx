...
const NavDesktop = ({ pathname }: { pathname: string }) => {
  const baseLinkClass = 'mr-4 transition-all duration-300 hover:text-red-300';
  ...
  return (
    <div className='fixed w-full flex justify-between px-6 py-4 text-white md:text-xs bg-[#1f1d1d] z-999'>
      <div className='flex items-center justify-between font-medium text-md'>
        {navLinks.map(({ link, title }) => (
          <a
            key={link}
            href={link}
            className={pathname === link ? `${baseLinkClass} text-red-300` : baseLinkClass}
          >
            {title}
          </a>
        ))}
      ...
      <Link href="/" className='flex items-center self-center hide-between-md-lg'>
